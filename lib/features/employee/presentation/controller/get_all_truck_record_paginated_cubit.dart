...
@injectable
class GetAllTruckRecordPaginatedCubit extends Cubit<PaginationState<TruckRecordPaginatedEntity>> {
  ...
  Future<void> emitGetAllTrips({
    ...
    var response = await _employeeBaseRepository.getAllTruckRecordPaginated();
    response.fold(
      (l) {
            if (kDebugMode) {
          print(l);
       ...
       (r) {
        if (r.data != null) {
          var dataList = r.data!.data.where((element) => element != null).cast<TruckRecordPaginatedEntity>().toList();
