...
@injectable
class TripArchiveCubit extends Cubit<PostState<BaseEntity>> {
 final EmployeeBaseRepository _employeeBaseRepository;
  TripArchiveCubit(
    this._employeeBaseRepository,
  ) : super(const PostState.idle());

  Future<void> emitTripArchive({required TripArchiveParams tripArchiveParams}) async {
    emit(const PostState.loading());
    final response = await _employeeBaseRepository.tripArchive(tripArchiveParams:tripArchiveParams);
