...
@injectable
class AddTripCubit extends Cubit<PostState<BaseEntity>> {
 final EmployeeBaseRepository _employeeBaseRepository;
  AddTripCubit(
    this._employeeBaseRepository,
  ) : super(const PostState.idle());

  Future<void> emitAddTrip({required AddTripParams addTripParams}) async {
    emit(const PostState.loading());
    final response = await _employeeBaseRepository.addTrip(addTripParams:addTripParams);
