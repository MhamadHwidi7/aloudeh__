...
@injectable
class GetAllClosedTripsPaginatedCubit extends Cubit<PaginationStateTest<ClosedTripsPaginatedEntity>> {
  final EmployeeBaseRepository _employeeBaseRepository;
  int currentPage = 1;
  int? lastPage;
  List<ClosedTripsPaginatedEntity?> _trips = [];

  GetAllClosedTripsPaginatedCubit(
  ...
  Future<void> emitGetAllClosedTrips({
    ...
    var response = await _employeeBaseRepository.getAllClosedTrips(currentPage);
    response.fold(
      (l) {
        if (kDebugMode) {
          print(l);
      ...
      (r) {
        if (r.data != null) {
          ...
          emit(PaginationStateTest.success(data: _trips.where((element) => element != null).cast<ClosedTripsPaginatedEntity>().toList(), canLoadMore: currentPage));
        }
      },
    );
  }

  void _addIncomingDataToClassMemberData(bool loadMore, BasePaginationEntity<PaginationEntity<ClosedTripsPaginatedEntity?>> model) {
    if (loadMore) {
