...
@injectable
class ArchiveTripCubit extends Cubit<PostState<BaseEntity>> {
 final EmployeeBaseRepository _employeeBaseRepository;
  ArchiveTripCubit(
    this._employeeBaseRepository,
  ) : super(const PostState.idle());

  Future<void> emitArchiveTrip({required ArchiveTripParams archiveTripParams}) async {
    emit(const PostState.loading());
    final response = await _employeeBaseRepository.archiveTrip(archiveTripParams);
