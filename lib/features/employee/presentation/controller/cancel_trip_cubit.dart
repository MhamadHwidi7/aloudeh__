...
@injectable
class CancelTripCubit extends Cubit<PostState<BaseEntity>> {
 final EmployeeBaseRepository _employeeBaseRepository;
  CancelTripCubit(
    this._employeeBaseRepository,
  ) : super(const PostState.idle());

  Future<void> emitCancelTrip({required CancelTripParams cancelTripParams}) async {
    emit(const PostState.loading());
    final response = await _employeeBaseRepository.cancelTrip(cancelTripParams:cancelTripParams);
