...
@injectable
class AddCompliantCubit extends Cubit<PostState<BaseEntity>> {
 final EmployeeBaseRepository _employeeBaseRepository;
  AddCompliantCubit(
    this._employeeBaseRepository,
  ) : super(const PostState.idle());

  Future<void> emitAddCompliant({required AddCompliantParams addCompliantParams}) async {
    emit(const PostState.loading());
    final response = await _employeeBaseRepository.addCompliant(addCompliantParams:addCompliantParams);
