...
@injectable
class GetAllGoodsPaginatedCubit extends Cubit<PaginationStateTest<GetAllGoodPaginatedEntity>> {
  ...
  GetAllGoodsPaginatedCubit(this._baseWarehouseRepository)
  ...
  Future<void> emitGetAllGoods({
    ...
    var response = await _baseWarehouseRepository.getAllGoodPaginated(currentPage);
    response.fold(
      (l) {
        if (kDebugMode) {
          print(l);
