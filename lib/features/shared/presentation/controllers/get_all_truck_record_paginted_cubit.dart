...
@injectable
class GetAllTruckRecordPaginatedSharedCubit extends Cubit<PaginationSharedState<TruckRecordPaginatedSharedEntity>> {
  final SharedBaseRepository _sharedBaseRepository;
  int currentPage = 1;
  int? lastPage;
  List<TruckRecordPaginatedSharedEntity?> _trucks = [];
  bool canLoadMoreData = true;

  GetAllTruckRecordPaginatedSharedCubit(this._sharedBaseRepository)
      : super(const PaginationSharedState.loading());

  Future<void> emitGetAllTruckRecord({
    bool loadMore = false,
  }) async {
    if (!canLoadMoreData) {
      return;
   ...
   final params = PaginatedParams(page: currentPage);
    var response = await _sharedBaseRepository.getAllTruckRecordPaginated(paginatedParams:params);
    response.fold(
      (l) {
        if (kDebugMode) {
          print(l);
      ...
      (r) {
        if (r.data != null) {
          ...
          emit(PaginationSharedState.success(
            data: _trucks.where((element) => element != null).cast<TruckRecordPaginatedSharedEntity>().toList(),
  ...
  void _addIncomingDataToClassMemberData(
      bool loadMore, BasePaginationEntity<PaginationEntity<TruckRecordPaginatedSharedEntity?>> model) {
    if (loadMore) {
