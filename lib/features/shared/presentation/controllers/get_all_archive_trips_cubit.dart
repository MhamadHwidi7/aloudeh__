...
@injectable
class GetAllArchiveTripsPaginatedSharedCubit extends Cubit<PaginationSharedState<ArchiveTripsPaginatedSharedEntity>> {
  final SharedBaseRepository _sharedBaseRepository;
  int currentPage = 1;
  int? lastPage;
  List<ArchiveTripsPaginatedSharedEntity?> _trips = [];
  bool canLoadMoreData = true;

  GetAllArchiveTripsPaginatedSharedCubit(this._sharedBaseRepository)
  ...
  Future<void> emitGetAllArchiveTrips({
   ...
   final params = PaginatedParams(page: currentPage);
    var response = await _sharedBaseRepository.getAllArchiveTrips(paginatedParams:params);
    response.fold(
      (l) {
        if (kDebugMode) {
          print(l);
      ...
      (r) {
        if (r.data != null) {
          ...
          emit(PaginationSharedState.success(
            data: _trips.where((element) => element != null).cast<ArchiveTripsPaginatedSharedEntity>().toList(),
  ...
  void _addIncomingDataToClassMemberData(
      bool loadMore, BasePaginationEntity<PaginationEntity<ArchiveTripsPaginatedSharedEntity?>> model) {
    if (loadMore) {
