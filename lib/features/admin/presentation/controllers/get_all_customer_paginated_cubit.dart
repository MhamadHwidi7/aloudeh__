...
@injectable
class GetAllCustomersPaginatedCubit extends Cubit<PaginationState<GetCustomerAdminPaginatedEntity>> {
  final AdminBaseRepository _adminBaseRepository;
  int currentPage = initialPage;
  bool canLoadMoreData = true;

  GetAllCustomersPaginatedCubit(
  ...
  Future<void> emitGetAllBranches({
    ...
    var response = await _adminBaseRepository.getAllCustomers();
    response.fold(
      (l) {
            if (kDebugMode) {
          print(l);
       ...
       (r) {
        if (r.data != null) {
          var dataList = r.data!.data.where((element) => element != null).cast<GetCustomerAdminPaginatedEntity>().toList();
